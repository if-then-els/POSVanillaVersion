...
exports.registerOwner = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Owner registered successfully", owner: newOwner });
  } catch (error) {
    console.error(error);
...
exports.logOwner = async (req, res) => {
  try {
    ...
    if (!owner) {
      return res
        .status(400)
        .json({ message: "Owner not found,check your credentials" });
    }
    ...
    res.status(200).json({
      ...
      owner: {
        id: owner._id,
        ownersName: owner.ownersName,
        ownersEmail: owner.ownersEmail,
        ownersPhone: owner.ownersPhone,
        ownersNationalIdentificationNumber:
          owner.ownersNationalIdentificationNumber,
      },
      token,
    });
  } catch (error) {
